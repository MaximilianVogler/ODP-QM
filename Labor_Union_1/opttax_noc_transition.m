function [welfare_mat,gg,w_t,pi_t,taulvec,flag,various] = opttax_noc_transition(gg_initial)
...
pi_new = 0.5*ones(N,1); % Guess tax sequence
...
pi_it=zeros(N,maxit_price_seq);
taul_it=zeros(N,maxit_price_seq);
...
pi_diff=zeros(N,maxit_price_seq);
%s_it=zeros(I,J,N);
...
% Compute tax
for n=1:N
taulvec(n) = 1/((1/(1-(1-theta)*RS))+(theta*RS/(1-(1-theta)*RS)*(1-theta)*RS/(1-RS)*pi_t(n)));
end
taul_it(:,it) = taulvec;
...
pi_new = pi_t.*(1+min(adj*pi_diff(:,it),0.5));
...
maxpidiff = max(abs(pi_diff(:,it)));

% Break if maximum ES low enough
if abs(maxESdiff)<crit_price_seq & abs(maxpidiff)<crit_price_seq_1
    break;
end
...
fprintf('    %d           %8.6f       %8.6f     %8.6f \n',it,maxwagediff,maxESdiff,maxpidiff);
