...
% fprintf('    %d       ',it);
% fprintf('%7.5f      ',w);
taul = 1/((1/(1-(1-theta)*RS))+(theta*RS/(1-(1-theta)*RS)*(1-theta)*RS/(1-RS)*pi));
...
%pi1 = ((1-ddconstrained(:))nn_unconstrained(:).*gg)'*da_stacked*dz/ld;
lab_constr = (1-pop_share)*(ddprod(:).*(1-ddconstrained(:)).*nn(:).*gg)'*da_stacked*dz;
pi1 = lab_constr/ld;
pi_diff=pi1-pi;
...
w = w*(1-min(adj*ES,0.9)); % Adjust based on ES (ES>0, then w should decrease)
...
pi = pi.*(1+min(adj*pi_diff,0.5));

% Break if both achieve success
if abs(ES)<crit_price & abs(pi_diff)<crit_price_1
    break;
end
