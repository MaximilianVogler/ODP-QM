...
paretoweight_workers = 2/3; %Pareto weight on workers
...
tau0_vec_tr = linspace(0,0.05,10)';
taubar_vec_tr = linspace(0,0.05,10)';
gal_vec_tr = [0];

% Vectors of parameters for optimal tax schedule
tax_vec_ss = linspace(-0.15,-0.1,20)'; %Testing for optimal steady state tax

tau0_vec = linspace(-0.35,-0.25,10)'; %Initial tax rate
taubar_vec = linspace(0.15,0.25,10)'; %Final tax rate
halflives_vec = [12 13 14];%[5 7 10]; %Tax schedule half life
...
maxit_vf=200;
maxit_dist=200;
crit_price = 10^(-4); %Convergence criterion steady state w
crit_price_1 = 10^(-4);
crit_price_seq = 10^(-4); %Convergence criterion sequence w
crit_price_seq_1 = 10^(-4);
crit_dist = 10^(-8);
crit_vf = 10^(-8);
crit_transfer = 10^(-5);
...
wmin0=1;
wmax0=4;
%Initial guess for fraction of constrainted entrepreneurs in SS
pimax=1;
pimin=0;
% tau_init=-0.105/2;
% pi0=((1/tau_init)-1/(1-(1-theta)*RS))/((theta*RS/(1-(1-theta)*RS))*((1-theta)*RS/(1-RS)));
