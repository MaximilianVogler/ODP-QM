...
kk=min(la*aa,(zz*Aprod).^(1/(1-al-be))*(al/((1-credk)*rstar+d))^((1-be)/(1-al-be))*...
...
Pi=yyprod-((1-credk)*rstar+d).*kk-w*nn; %Profits
ddprod = Pi>0;
M=ddprod.*Pi; %Income of entrepreneurs

kk_unconstrained = (zz*Aprod).^(1/(1-al-be))*(al/((1-credk)*rstar+d))^((1-be)/(1-al-be))*...
    (be/w)^(be/(1-al-be))+fk;
...
transfer = -((1-pop_share)/pop_share)*credk*rstar*(ddprod(:).*kk(:).*gg)'*da_stacked*dz;
ls=ls/pop_share; % Per capita labor
welfare_entrepreneurs = (v(:).*gg)'*da_stacked*dz;  % Per capita entrepreneur welfare
consumption_workers = w*ls+transfer; %Per capita worker consumption level (=income)
%transfer = taul*w*ls;
if preftype==1
