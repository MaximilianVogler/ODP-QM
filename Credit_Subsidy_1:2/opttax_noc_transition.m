...
for n=N:-1:1
    ...
    kk=min(la*aa,(zz*Aprod).^(1/(1-al-be))*(al/((1-credkvec(n))*rstar+d))^((1-be)/(1-al-be))*...
...
taxrev = zeros(N,1);
for m=1:N
    ddprod2 = reshape(dddprod(:,:,m),I*J,1);
    kks2 = reshape(kkk(:,:,m),I*J,1);
    taxrev(m) = (ddprod2(:).*kks2(:).*gg{m})'*da_stacked*dz;
end
transfervec = -((1-pop_share)/pop_share)*credkvec.*rstar.*taxrev;
lsvec=ls_it(:,it)/pop_share; %Per capita labor supply
consumption_workers = w_t.*lsvec+transfervec; %Total consumption of workers
