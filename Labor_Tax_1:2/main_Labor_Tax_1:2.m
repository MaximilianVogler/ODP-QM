...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% OPTIMAL STEADY STATE TAX RATE         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if run_optss==1
    
disp('--------------------------------');
disp('OPTIMAL STEADY STATE TAX RATE');
disp('--------------------------------');

% Find optimal tax by iteration over grid
run opttax_noc_parameters.m;
g_optss_struct = cell(length(tax_vec_ss));
welfare_vec_ss = zeros(length(tax_vec_ss),1);
disp(' Tax      Welfare total    Welfare workers    Welfare entrepreneurs');
for ittax=1:length(tax_vec_ss)
    taul=tax_vec_ss(ittax);
    gg_sparse=sparse(ones(I*J,1)./(I*J*dz*da_stacked)); %Uniform guess
    %fprintf('Trying tax = %5.3f \n',taul);
    ...
    fprintf('%5.3f        %7.5f          %7.5f             %7.5f \n',taul,welfare_tot_ss,welfare_workers,welfare_entrepreneurs);
end
disp('Done!');
...
fprintf('Optimal tax = %5.3f \n',opttaul_ss);
g_a_opttax = sum(g.*dz,2);


% Find no tax results
disp('RESULTS WITH NO TAX');
taul=0;
...
legend('No tax',sprintf('Optimal tax = %d',opttaul_ss));
...
% Save figure of welfare as function of tax
H=figure;
set(gcf,'Visible','off');
plot(tax_vec_ss,welfare_vec_ss,'--b');
title('Worker welfare as function of tax');
xlabel('Tax $\tau_l$','interpreter','latex');
...
% Option 3: Start from steady state distribution under the optimal steady state tax
% g0=g_optss;
...
% Save optimal flat tax schedule
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% OPTIMAL TRANSITION TAX SCHEDULE       %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if run_findpath==1
    
disp('-------------------------------------------');
disp('FINDING THE OPTIMAL TRANSITION TAX SCHEDULE');
...
for ii=1:gp_gal
    ...
    title(sprintf('Welfare by tax schedule, half life = %d',halflives_vec(ii)));
    set(gca,'FontSize',14);
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PLOTS FOR OPTIMAL TAX AND NO TAX      %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if run_optpath==1
...
[welfare_mat,gg,w_t,~,ls_it] = opttax_noc_transition(gg_guess,gg_initial,taulvec);
% Save results
...
% RUN AND SAVE RESULTS FOR FIXED TAX AT STEADY STATE OPTIMUM
disp('FIXED TAX: STEADY STATE OPTIMUM');
% Find transition path
taul_bar = opttaul_ss; 
taul0 = opttaul_ss;
gal = 0;
...
% RUN AND SAVE RESULTS FOR OPTIMAL FLAT TAX
disp('FIXED TAX: STEADY STATE OPTIMUM');
% Find transition path
...
% RUN AND SAVE RESULTS FOR OPTIMAL FLAT TAX
% disp('FIXED TAX: OPTIMAL FLAT TAX');
% % Find transition path
% taul_bar = opttaul_flat; 
% taul0 = opttaul_flat;
% gal = 0;
...
xlabel('Time');
title(sprintf('Optimal tax schedule, half life = %5.3f',halflife_opt));
set(gca,'FontSize',14);
...
fprintf(fid,'\\begin{center} \n \\begin{tabular}{l | c | c | c | c } \n \\hline \n');
fprintf(fid,' & No tax & Optimal policy & Optimal SS tax & Optimal Flat tax \\\\ \n');
fprintf(fid,'\\hline \n');
...
fprintf(fid,'\\begin{table}[h!]\n');
fprintf(fid,'\\begin{center} \n \\begin{tabular}{l | c | c } \n \\hline \n');
fprintf(fid,' & Constant $\\tau_0$ & Constant $\\bar{\\tau}$ \\\\ \n');
fprintf(fid,'\\hline \n');
...
fprintf(fid,'Range of initial tax rate tested & $\\tau_0$ & [%5.3f,%5.3f] \\\\ \n',tau0_vec(1),tau0_vec(end));
fprintf(fid,'Range of final tax rate tested & $\\bar{\\tau}$ & [%5.3f,%5.3f] \\\\ \n',taubar_vec(1),taubar_vec(end));
...
fprintf(fid,'Optimal steady state tax rate = %5.3f \n',opttaul_ss);
% Welfare as function of tax
...
% PRINT Welfare by tax schedule
if print_findpath==1
    % Print 3D plots of welfare by tax schedule parameters
...
copyfile('main_Labor_Tax_1:2.m',path_docs);
